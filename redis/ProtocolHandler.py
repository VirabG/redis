import re
LINE_SEPARATOR = '\r\n'
LSL = len(LINE_SEPARATOR)    # Line Separator Length
...
#########Vlad started
		...
		start_of_element = end_of_line_1 + LSL
		end_of_element = 0
		for i in range(n_elements):
			...
			if element_type == '+':
				...
				end_of_element = s.find(LINE_SEPARATOR, end_of_first_line + LSL)
				end_of_element += LSL
...
#---------------------------------------------------
	...
	def deserialize_dictionary(self, s):
		...
		start_of_key = end_of_line_1 + LSL
		end_of_key = 0
		start_of_value = 0
		end_of_value = 0
		...
		for i in range(n_elements):
			...
			if value_type == '+':
				...
				end_of_value = s.find(LINE_SEPARATOR, end_of_first_line + LSL)
				end_of_value += LSL
				...
				end_of_first_line = s.find(LINE_SEPARATOR, start_of_value)
				end_of_value = end_of_first_line + LSL + n_bytes
